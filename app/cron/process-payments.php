...
// Si no se ingresa como admin en http, debe existir arg1 pasado por linea de comandos con la clave correcta, de lo contrario se deniega el acceso.
if(!$loggedUser) {
	if(isset($argv[1])) { 
...
if(!$con) exit;
$config = include(ROOT_PUBLIC."global_scripts/config.php");

require_once(ROOT."app/lib/AfipWs/AfipWs.php");
...
function saveConfig($config) {
	file_put_contents(ROOT_PUBLIC."global_scripts/config.php", "<?php return ".var_export($config, true)."; ?>");
}
