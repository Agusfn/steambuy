...
class Mail
{
	...
	function __construct() {
		...
		$this->TEMPLATE_PATH = ROOT."app/template/email/";
	}
	
	public function prepare_email($email_type, $data) { 
		
		if(!preg_match("/^[\/a-z0-9_]*$/i", $email_type)) return false;
		else if(!file_exists($this->TEMPLATE_PATH.$email_type.".php")) return false;
		
		$body_frame = file_get_contents($this->TEMPLATE_PATH."main.html");
		
		ob_start();
		require($this->TEMPLATE_PATH.$email_type.".php");
